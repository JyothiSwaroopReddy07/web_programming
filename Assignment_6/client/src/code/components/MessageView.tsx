...
const MessageView = ({ state }): JSX.Element => (
  ...
  <form>
    ...
    { state.currentView === "message" && state.messageBody.isSpam &&
      <div className="messageInfoField" style={{backgroundColor:"red"}}>Spam</div>
    }
    ...
    <TextField margin="dense" id="messageBody" variant="outlined" fullWidth={ true } multiline={ true } rows={ 12 }
      value={ state.messageBody.body } disabled={ state.currentView === "message" }
      InputProps={{ style : { color : "#000000" } }} onChange={ state.fieldChangeHandler } />
